...
/*
void GraspPlannerWindow::ParseXML()
{
    rapidxml::file<> xmlFile(objectFile.c_str());
    rapidxml::xml_document<> doc;
    doc.parse<0>(xmlFile.data());
    rapidxml::xml_node<>* manipulationObjectNode = doc.first_node("ManipulationObject");

    if (manipulationObjectNode) {
        // Find the name attribute within ManipulationObject
        rapidxml::xml_attribute<>* nameAttribute = manipulationObjectNode->first_attribute("name");

        std::ifstream databaseFile("DataBase2.json");
        json database;
        databaseFile >> database;
        if (database.find(nameAttribute->value()) != database.end()) {
        ...
        if (nameAttribute) {
            ...
            std::string objectName = nameAttribute->value();
            std::cout << "Object Name: " << objectName << std::endl;
        } else {
            std::cerr << "Attribute 'name' not found within ManipulationObject." << std::endl;
        }
    } else {
        std::cerr << "Element 'ManipulationObject' not found in the XML." << std::endl;
    }
}
